...
async def GetDisplayName(message, userid, bot):
  ...
  if not Origin:
    await DMHelper.DMUser(message, f"Something went wrong retrieving the server ID")
    return
  ...
  try:
    ...
    await DMHelper.DMUser(message, f"Something went wrong retrieving this users display name, perhaps they have left the server")
  try:
    if member_obj:
    ...
    await DMHelper.DMUser(message, f"Something went wrong retrieving the users' display name")
    return
