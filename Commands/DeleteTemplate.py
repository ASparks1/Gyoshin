...
async def DeleteTemplate(message, bot):
  ...
  c = conn.cursor()
  c.execute(f"SELECT Name FROM Templates WHERE CreatorUserID = (?) AND Origin = (?)", (UserID, Origin,))
  rows = c.fetchall()
  ...
  if rows:
    ...
    while not ValidTemplateName:
      await DMHelper.DMUserByID(bot, UserID, f"Please provide the name of the template you wish to delete")
      try:
        ...
        row = c.fetchone()
        if not row:
          await DMHelper.DMUserByID(bot, UserID, f"Please enter a valid template name.")
          continue
        elif row:
          ...
          try:
            ...
            while not CanDelete:
              ...
              if response.content == "Y" or response.content == "y" or response.content == "Yes" or response.content == "yes":
                CanDelete = 'yes'
                c.execute(f"DELETE FROM Templates WHERE Name = (?) AND Origin = (?)",(TemplateName, Origin,))
                conn.commit()
                conn.close()
                await DMHelper.DMUserByID(bot, UserID, f"Template succesfully deleted.")
