  ...
  if not usercheck:
    ...
    if RoleName == 'tank':
      if NrOfTanksSignedUp == NrOfTanksRequired:
        ...
        c.execute(f"SELECT ID FROM RaidReserves WHERE RaidID = (?) AND UserID = (?)",(RaidID, UserID))
        row = c.fetchone()
        if row:
          await DMHelper.DMUserByID(bot, UserID, f"You're already on the reserves list for this run would you like to withdraw from the reserves? (Y/N).")
          ...
          while not WithdrawFromReserves:
            try:
              ...
              conn.close()
              await DMHelper.DMUserByID(bot, UserID, f"Your request has timed out, please click the button again if you still wish to withdraw from the reserves for this run.")
              return
          
        await DMHelper.DMUserByID(bot, UserID, f"This run already has the required number of tanks, would you like to be put on the reserve list? (Y/N).")
        TankReserve = None
        while not TankReserve:
          try:
            ...
            conn.close()
            await DMHelper.DMUserByID(bot, UserID, f"Your request has timed out, please click the button again if you still wish to join the reserves for this run.")
            return
      else:
        try:        
      ...
      if NrOfDpsSignedUp == NrOfDpsRequired:
        ...
        await DMHelper.DMUserByID(bot, UserID, f"This run already has the required number of dps, would you like to be put on the reserve list? (Y/N).")
        DPSReserve = None
      ...
      if NrOfHealersSignedUp == NrOfHealersRequired:
        ...
        await DMHelper.DMUserByID(bot, UserID, f"This run already has the required number of healers, would you like to be put on the reserve list? (Y/N).")
        HealerReserve = None
    ...
    try:
      ...
      c.execute(f"DELETE FROM RaidReserves where RaidID = (?) AND UserID = (?) AND Origin = (?)",(RaidID, UserID, Origin))
      # Add user to run
